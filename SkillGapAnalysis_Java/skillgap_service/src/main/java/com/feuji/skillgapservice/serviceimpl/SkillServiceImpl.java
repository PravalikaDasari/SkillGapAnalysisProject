...
public class SkillServiceImpl implements SkillService {
	...
	public SkillBean saveSkill(SkillBean skillBean) {
		...
		try {
			// Check if the skill already exists in the database by name
	...
	public void updateStatusBySkillId(Integer skillId, Byte status) throws RecordNotFoundException {
		Optional<SkillEntity> findById = skillRepository.findById(skillId);
		if (findById.isPresent()) {
			SkillEntity skillEntity = findById.get();
			skillEntity.setStatus(status);
